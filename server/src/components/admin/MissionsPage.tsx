          ...
          <div className="space-y-4">
            ...
            {missionType === 'quiz' && (
              <>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">인증 장소 (선택사항)</label>
                  <select value={form.placeId} onChange={(e) => setForm({ ...form, placeId: Number(e.target.value) })}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    <option value={0}>선택 안 함</option>
                    {places.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
                  <p className="text-xs text-gray-500 mt-1">장소를 선택하면 해당 위치 근처에서만 퀴즈를 풀 수 있습니다</p>
                </div>
              </>
            )}
